...
function PublicGarage.EnsurePendingBill(source, record, garageId)
...
function PublicGarage.GetVehicles(source, garageId)
    ...
    for _, row in ipairs(rows) do
        PublicGarage.EnsurePendingBill(source, row, garageId)
...
function PublicGarage.SpawnVehicle(source, garageId, plate)
    ...
    local fee = PublicGarage.EnsurePendingBill(source, record, garageId)
    if fee > 0 and settings().allowSpawnWithUnpaidFees ~= true then
        ...
        return ServerUtils.Failure('unpaid_garage_fee', Locale.public_unpaid_fee_banking, { unpaidFee = fee, plate = plate })
    end
...
function PublicGarage.RefreshBill(source, plate)
    ...
    local fee = PublicGarage.EnsurePendingBill(source, record, record.garage_id)
