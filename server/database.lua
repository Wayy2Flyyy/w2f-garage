...
Database._memoryPublicBills = Database._memoryPublicBills or {}
Database._memoryPublicBillSeq = Database._memoryPublicBillSeq or 0
...
function Database.GetPublicGarageBillsByOwner(identifier, status)
    ...
    local rows={}
    for _,b in pairs(Database._memoryPublicBills) do if b.owner_identifier==identifier and (not status or b.status==status) then rows[#rows+1]=b end end
    return rows
end
...
function Database.GetPendingPublicGarageBill(plate)
    ...
    for _,b in pairs(Database._memoryPublicBills) do if b.plate==plate and b.status=='pending' then return b end end
    return nil
end
...
function Database.CreateOrUpdatePublicGarageBill(entry)
    return Database.UpsertPublicGarageBill(entry)
end

function Database.CancelPublicGarageBill(id)
    return Database.UpdatePublicGarageBillStatus(id, 'cancelled')
end

function Database.SetPublicCurrentBill(plate, billId)
    return Database.SetPublicVehicleBill(plate, billId, nil, os.time())
end

function Database.GetPublicCurrentBill(plate)
    local row = Database.GetPublicVehicle(plate)
    if not row then return nil end
    if not row.current_bill_id then return nil end
    if Database.UsePublicPersistence() then
